...
abstract class PersistentFsmSpec(extensionClass: Class[_], database: String, extendedConfig: String = "|")
  ...
  "A mongo persistence driver" should
    "support persistent FSM purchase sequence" in withConfig(config(extensionClass), "pekko-contrib-mongodb-persistence-journal", "persistent-fsm") { case (as, config) =>
  ...
  it should
    "support persistent FSM shop and discard" in withConfig(config(extensionClass), "pekko-contrib-mongodb-persistence-journal", "persistent-fsm") { case (as, config) =>
      implicit val system: ActorSystem = as
  ...
  it should
    "support persist and recovery of FSM cart" in withConfig(config(extensionClass), "pekko-contrib-mongodb-persistence-journal", "persistent-fsm") { case (as, config) =>
      implicit val system: ActorSystem = as
