...
import pekko.contrib.persistence.mongodb.SnapshottingFieldNames._
...
abstract class MongoPersistenceDriver(as: ActorSystem, config: Config)
  ...
  val DEFAULT_DB_NAME = "pekko-persistence"
  ...
  private val canSuffixCollectionNamesBuilder: Option[CanSuffixCollectionNames] = suffixBuilderClassOption match {
    case Some(suffixBuilderClass) if suffixBuilderClass.trim.nonEmpty =>
  ...
  private[mongodb] val querySideDispatcher = actorSystem.dispatchers.lookup("pekko-contrib-persistence-query-dispatcher")
  ...
  def suffixSeparator: String = settings.SuffixSeparator match {
    case str if str.nonEmpty => validateMongoCharacters(settings.SuffixSeparator).substring(0, 1)
    case _ => "_"
  }
