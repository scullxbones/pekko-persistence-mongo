...
abstract class JournalTaggingSpec(extensionClass: Class[_], database: String, extendedConfig: String = "|")
  ...
  "Several taggers" should "persist tags" in withConfig(config(extensionClass), "pekko-contrib-mongodb-persistence-journal", "tagging-test") { case (as, _) =>
  ...
  "Query by tag" should "return events that are tagged" in withConfig(config(extensionClass), "pekko-contrib-mongodb-persistence-journal", "tagging-test") { case (as, _) =>
    implicit val system: ActorSystem = as
  ...
  "Query by tag" should "not return events that don't match tags" in withConfig(config(extensionClass), "pekko-contrib-mongodb-persistence-journal", "tagging-test") { case (as, _) =>
    implicit val system: ActorSystem = as
    implicit val am: Materializer = Materializer(as)
    val readJournal = PersistenceQuery(system).readJournalFor[CurrentEventsByTagQuery](MongoReadJournal.Identifier)
  ...
  "Query by tag" should "not allow unsupported offset types" in withConfig(config(extensionClass), "pekko-contrib-mongodb-persistence-journal", "tagging-test") { case (as, _) =>
    implicit val system: ActorSystem = as
    implicit val am: Materializer = Materializer(as)
    val readJournal = PersistenceQuery(system).readJournalFor[CurrentEventsByTagQuery](MongoReadJournal.Identifier)
  ...
  "Current by tag" should "return live events as persisted" in withConfig(config(extensionClass), "pekko-contrib-mongodb-persistence-journal", "tagging-test") { case (as, _) =>
    implicit val system: ActorSystem = as
    implicit val am: Materializer = Materializer(as)
